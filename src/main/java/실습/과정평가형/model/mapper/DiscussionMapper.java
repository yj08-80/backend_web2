...
public interface DiscussionMapper { // interface start
    ...
    // 토론 글 작성
    @Insert( "INSERT INTO discussion ( movie_id, content, password) VALUES ( #{movie_id}, #{content}, #{password})")
    public int disWrite( DiscussionDto discussionDto );

    // 토론 글 삭제
    @Delete( "DELETE FROM discussion WHERE id = ( #{id} ) AND password = ( #{password} ) ")
    public boolean disDelete( int id , String password );

    // 영화별 토론 전체 조회
    @Select( "SELECT * FROM discussion where movie_id = ( #{movie_id} )")
